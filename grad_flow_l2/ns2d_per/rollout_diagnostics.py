...
def _scalar_vorticity(u: torch.Tensor) -> torch.Tensor:
    ...
    if u.dim() == 5 and u.shape[2] == 1:
        return u[:, :, 0]
    raise ValueError(
        f"expected vorticity trajectory shape (batch,time,n_x,n_y), got {tuple(u.shape)}"
    )


def _wavenumber_grids(
    n_x: int, n_y: int, device, dtype
...
def _radial_energy_spectrum(u: torch.Tensor, area: float) -> torch.Tensor:
    ...
    velocity_power = torch.where(
        angular_k_sq > 0,
        vorticity_power / angular_k_sq,
        torch.zeros_like(vorticity_power),
    )
    ...
    spectrum = torch.zeros(
        flat_power.shape[0], n_shells, device=u.device, dtype=u.real.dtype
    )
    spectrum.scatter_add_(dim=1, index=flat_shell, src=flat_power)
    return spectrum.reshape(*u.shape[:-2], n_shells)


def _scalar_relative_diagnostics(
    pred: torch.Tensor, ref: torch.Tensor, eps: float
) -> tuple[np.ndarray, float, np.ndarray, np.ndarray]:
    abs_ref = ref.abs()
    curve_samples = (pred - ref).abs() / (abs_ref + eps)
    curve = torch.mean(curve_samples, dim=0).detach().cpu().numpy().astype(np.float64)
    samples = torch.sqrt(
        torch.sum((pred - ref).square(), dim=1) / (torch.sum(ref.square(), dim=1) + eps)
    )
    samples_np = samples.detach().cpu().numpy().astype(np.float64)
    aggregate = float(np.nanmean(samples_np))
    curve_samples_np = curve_samples.detach().cpu().numpy().astype(np.float64)
    return curve, aggregate, samples_np, curve_samples_np


def compute_rollout_diagnostics(
    u_pred: torch.Tensor, u_ref: torch.Tensor, area: float
) -> Dict[str, object]:
    """Compute invariant and spectrum rollout diagnostics for predicted snapshots only."""
    u_pred = u_pred[:, 1:]
    u_ref = u_ref[:, 1:]
    ens_curve, ens_agg, ens_samples, ens_curve_samples = _scalar_relative_diagnostics(
        _enstrophy(u_pred, area), _enstrophy(u_ref, area), eps=1e-24
    )
    pal_curve, pal_agg, pal_samples, pal_curve_samples = _scalar_relative_diagnostics(
    ...
    spec_rel = torch.sqrt(
        torch.sum((spec_pred - spec_ref).square(), dim=-1)
        / (torch.sum(spec_ref.square(), dim=-1) + 1e-24)
    )
    spec_curve = torch.mean(spec_rel, dim=0).detach().cpu().numpy().astype(np.float64)
    spec_samples_t = torch.sqrt(
        torch.sum((spec_pred - spec_ref).square(), dim=(1, 2))
        / (torch.sum(spec_ref.square(), dim=(1, 2)) + 1e-24)
    )
