...
class DeterministicStateEncoder1D(nn.Module):
    ...
    def forward(self, u: torch.Tensor, f: torch.Tensor | None = None) -> torch.Tensor:
        u_b, squeeze = _ensure_batch_state_1d(u, name="u")
        if u_b.shape[1] != self.n_x:
            raise ValueError(f"u width must be {self.n_x}, got {u_b.shape[1]}")

        if self.use_grad_features:
            x = torch.stack([u_b, self._grad_feat(u_b)], dim=1)
        else:
            x = u_b.unsqueeze(1)
