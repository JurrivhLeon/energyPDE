...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        "--output-dir", type=str, default="grad_flow_l2/euler1d/outputs_fno/eval"
    )
    ...
    p.add_argument(
        ...
        type=float,
        default=0.0,
        help="Fallback FNO lift-noise std when args.json is unavailable; inactive during eval.",
    )
    p.add_argument(
        "--lift-noise-corr-length",
        type=float,
        default=1.0,
        help="Fallback Matern correlation length for FNO lift noise.",
    )
    p.add_argument(
        "--lift-noise-decay-s",
        type=float,
        default=2.0,
        help="Fallback spectral decay exponent s for FNO lift noise.",
    )
    return p.parse_args()
...
def _build_model(n_x: int, dt: float, args, train_args) -> FNO1D:
    return FNO1D(
        ...
        lift_noise_std=float(
            getattr(train_args, "lift_noise_std", args.lift_noise_std)
        ),
        lift_noise_corr_length=float(
            getattr(train_args, "lift_noise_corr_length", args.lift_noise_corr_length)
        ),
        lift_noise_decay_s=float(
            getattr(train_args, "lift_noise_decay_s", args.lift_noise_decay_s)
        ),
    )
