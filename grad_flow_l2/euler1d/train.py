"""Train deterministic latent Markov models on 1D Euler data."""
...
try:
    ...
    from ..latent_markov_mc import (
        FNOProximalStepSimulator1D,
        LatentMarkovModel1D,
        ProximalStepSimulator1D,
        StateDecoder1D,
        StateEncoder1D,
    )
    ...
    from grad_flow_l2.latent_markov_mc import (
        FNOProximalStepSimulator1D,
        LatentMarkovModel1D,
        ProximalStepSimulator1D,
        StateDecoder1D,
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Train deterministic latent Markov model on 1D Euler data")
    ...
    p.add_argument("--transition-type", type=str, default="fno", choices=["cnn", "fno"])
    p.add_argument("--cnn-blocks", type=int, default=6)
    ...
    p.add_argument("--output-dir", type=str, default="grad_flow_l2/euler1d/outputs")
    p.add_argument("--dry-run", action="store_true")
    return p.parse_args()


def _build_model(n_x: int, dt: float, boundary_condition: str, args: argparse.Namespace) -> LatentMarkovModel1D:
    use_forcing = not args.disable_forcing_channel
    width = args.hidden_channels if args.fno_width is None else args.fno_width
    encoder = StateEncoder1D(n_x=n_x, latent_channels=args.latent_channels, hidden_channels=args.hidden_channels,
                             n_blocks=args.enc_blocks, use_grad_features=not args.disable_u_grad_feature,
                             boundary_condition=boundary_condition, state_channels=STATE_CHANNELS)
    decoder = StateDecoder1D(n_x=n_x, latent_channels=args.latent_channels, hidden_channels=args.hidden_channels,
                             n_blocks=args.dec_blocks, boundary_condition=boundary_condition,
                             state_channels=STATE_CHANNELS)
    if args.transition_type == "cnn":
        transition = ProximalStepSimulator1D(n_x=n_x, latent_channels=args.latent_channels,
                                             hidden_channels=args.hidden_channels, n_blocks=args.cnn_blocks,
                                             use_forcing_channel=use_forcing, use_dt_channel=args.use_dt_channel,
                                             default_dt=dt, boundary_condition=boundary_condition)
    else:
        transition = FNOProximalStepSimulator1D(n_x=n_x, latent_channels=args.latent_channels,
                                                width=width, n_layers=args.fno_layers, modes=args.fno_modes,
                                                use_forcing_channel=use_forcing, use_dt_channel=args.use_dt_channel,
                                                use_grid_features=not args.disable_fno_grid,
                                                default_dt=dt, boundary_condition=boundary_condition)
    return LatentMarkovModel1D(encoder=encoder, decoder=decoder, transition=transition)


class EulerLatentMarkovTrainer:
    def __init__(self, model, dt: float, h: float, channel_weights, lambda_recon=1.0, lr=1e-4,
                 lr_step_size=100, lr_gamma=0.5, weight_decay=1e-5, grad_clip=1.0,
                 rollout_delta_clip: Optional[float] = 2.0, device="cpu", output_dir=None, show_epoch_pbar=True):
    ...
    def train_epoch(self, loader, epoch=None):
        self.model.train()
        meters = {k: AverageMeter() for k in ("loss", "loss_step", "loss_recon")}
        iterable = loader
        pbar = None
    ...
    def validate(self, step_loader, traj_loader=None):
        self.model.eval()
        meters = {k: AverageMeter() for k in ("val_loss", "val_loss_step", "val_loss_recon")}
    ...
    def fit(self, train_step_loader, val_step_loader, val_traj_loader=None, epochs=200, eval_interval=1, checkpoint_interval=25):
        ...
        for epoch in range(1, int(epochs)+1):
            ...
            if epoch % int(eval_interval) == 0:
                ...
                print(f"[Epoch {epoch:03d}] train_total={tr['loss']:.6f} train_step={tr['loss_step']:.6f} train_recon={tr['loss_recon']:.6f} val_total={val['val_loss']:.6f} val_step={val['val_loss_step']:.6f} val_recon={val['val_loss_recon']:.6f} val_rollout={val.get('val_rollout_rel_l2', float('nan')):.6f}")
            else:
                print(f"[Epoch {epoch:03d}] train_total={tr['loss']:.6f} train_step={tr['loss_step']:.6f} train_recon={tr['loss_recon']:.6f}")
...
def main(args):
    ...
    print(f"Grid: n_x={n_x}, channels={STATE_CHANNELS}, steps={n_steps}, dt={dt:.6f}, L={domain_length}, bc={bc}")
    ...
    trainer = EulerLatentMarkovTrainer(model, dt=dt, h=h, channel_weights=channel_weights,
                                      lambda_recon=args.lambda_recon, lr=args.lr,
