...
def channel_weighted_mse(u_pred: torch.Tensor, u_ref: torch.Tensor, channel_weights=None) -> torch.Tensor:
    if u_pred.shape != u_ref.shape:
        raise ValueError(f"u_pred and u_ref must have same shape, got {tuple(u_pred.shape)} vs {tuple(u_ref.shape)}")
    if channel_weights is None:
        return F.mse_loss(u_pred, u_ref)
    if u_pred.dim() != 3:
        raise ValueError(f"Expected state shape (batch,channels,n_x), got {tuple(u_pred.shape)}")
...
@torch.no_grad()
def rollout_model_1d(model, u0: torch.Tensor, f: torch.Tensor, n_steps: int, dt: float, delta_clip=None) -> torch.Tensor:
    squeeze = False
    if u0.dim() == 2:
        u0 = u0.unsqueeze(0)
        f = f.unsqueeze(0)
...
@torch.no_grad()
def rollout_vae_mean_1d(model, u0: torch.Tensor, f: torch.Tensor, n_steps: int, dt: float, delta_clip=None) -> torch.Tensor:
    squeeze = False
    if u0.dim() == 2:
        u0 = u0.unsqueeze(0)
        f = f.unsqueeze(0)
...
def relative_l2_error_1d(u_pred: torch.Tensor, u_ref: torch.Tensor, h: float) -> torch.Tensor:
    if u_pred.shape != u_ref.shape:
        raise ValueError(f"u_pred and u_ref must have same shape, got {tuple(u_pred.shape)} vs {tuple(u_ref.shape)}")
    diff = u_pred - u_ref
...
def spectral_h1_squared_1d(u: torch.Tensor, domain_length: float = 1.0) -> torch.Tensor:
    ...
    k = 2.0 * torch.pi * torch.fft.fftfreq(n_x, d=dx, device=u.device).to(dtype=real_dtype)
