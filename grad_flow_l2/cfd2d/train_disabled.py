"""Training entrypoint for periodic 2D compressible NS hidden-space gradient-flow models.

Uses the legacy grad_flow2d_mc (HiddenGradientFlowModel2D).
Kept for loading checkpoints trained with the original training script.
New training should use train.py (LatentMarkovModel2D from latent_markov_mc).
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train hidden-space gradient-flow model on periodic 2D compressible NS data")
    ...
    parser.add_argument("--energy-head-type",  type=str,   default="fno", choices=["local", "fno"])
    ...
    parser.add_argument("--output-dir",     type=str,  default="grad_flow_l2/cfd2d/outputs")
    parser.add_argument("--dry-run",        action="store_true")
    return parser.parse_args()


def _build_model(n_x: int, n_y: int, h_x: float, h_y: float, dt: float,
                 args: argparse.Namespace) -> HiddenGradientFlowModel2D:
    bc = "periodic"
    ...
    energy_head = EnergyHead2D(
        n_x=n_x, n_y=n_y, h_x=h_x, h_y=h_y,
        latent_channels=args.latent_channels,
        hidden_channels=args.hidden_channels,
    ...
    return HiddenGradientFlowModel2D(encoder=encoder, decoder=decoder,
                                     prox_step=prox_step, energy_head=energy_head)
