...
try:
    ...
    from grad_flow_l2.cfd2d.train_vae import (
        LatentVAETrainer2D,
        _rollout_vae_latent_mean,
        _rollout_vae_mean,
    )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Evaluate periodic 2D compressible NS latent VAE checkpoint"
    )
    ...
    parser.add_argument(
        "--output-dir", type=str, default="grad_flow_l2/cfd2d/outputs_vae/eval"
    )
...
def main(args: argparse.Namespace) -> None:
    ...
    trainer = LatentVAETrainer2D(
        ...
        transition_noise_scale=getattr(
            train_args,
            "transition_noise_scale",
            checkpoint.get("transition_noise_scale", 1.0),
        ),
    ...
    rollout_fn = (
        _rollout_vae_latent_mean if args.rollout_mode == "latent" else _rollout_vae_mean
    )
