...
try:
    ...
    from .train_disabled import _build_model   # uses grad_flow2d_mc (HiddenGradientFlowModel2D)
except ImportError:
    ...
    from grad_flow_l2.cfd2d.train_disabled import _build_model   # uses grad_flow2d_mc (HiddenGradientFlowModel2D)
...
def _evaluate_rollout_curves(model, traj_loader: DataLoader, device: str,
    ...
    for batch in traj_loader:
        ...
        u_pred = rollout_model_2d(model, u0=u0, f=f,
...
def _plot_samples(model, split, device: str, dt: float, t_final: float,
    ...
    for sample_id in sample_ids:
        ...
        pred = rollout_model_2d(model, u0=u0, f=f, n_steps=n_steps, dt=dt,
...
def main(args: argparse.Namespace) -> None:
    ...
    model = _build_model(n_x=n_x, n_y=n_y, h_x=1.0 / n_x, h_y=1.0 / n_y,
                         dt=dt, args=train_args).to(device)
    ...
    trainer = HiddenGradientFlowTrainer2D(model=model, dt=dt,
                                          h_x=1.0 / n_x, h_y=1.0 / n_y,
