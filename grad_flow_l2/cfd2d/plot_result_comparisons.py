"""Plot CFD2D rollout metric comparisons from trained checkpoints."""
...
TRAINING_HORIZON_END = 10.0

METHODS = (
    ("outputs_fno_nu8", "ood_eval", "Vanilla FNO", "-", "o"),
    ("outputs_fno_noisy_nu8", "ood_eval", "FNO+Noise", "--", "s"),
    ("outputs_ae_nu8", "ood_eval_t10", "FNO-AE", ":", "^"),
    ("outputs_vamo_nu8", "ood_eval_t10", "VAMO-FNO", "-.", "D"),
)
METRICS = (
    ("rel_l2_mean_all", "Relative L2"),
    ("rel_h1_mean_all", "Relative H1"),
)
...
def _apply_axes_style(ax: plt.Axes, x_values: np.ndarray) -> None:
    ...
    ax.set_xlabel("Rollout step", fontsize=11, labelpad=5)
    if x_values.size:
...
def plot_comparison(root: Path, output_dir: Path) -> Path:
    ...
    for method_dir_name, eval_dir_name, method_label, linestyle, marker in METHODS:
        csv_path = (
            _latest_run(root / method_dir_name)
            / eval_dir_name
            / "test_rollout_error_curve.csv"
        )
        ...
        for ax, (column, metric_label) in zip(axes, METRICS):
            ...
            ax.set_ylabel("Error", fontsize=11, labelpad=5)
    ...
    fig.suptitle("Error propogation in 2D CFD", y=0.99, fontsize=15)
