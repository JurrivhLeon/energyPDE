"""
Evaluation script for 1D Burgers latent VAE models.
...
4) Per-sample forcing/reference/prediction comparison plots.
"""
...
try:
    ...
    from grad_flow_l2.burgers.train_vae import rollout_vae_mean
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Evaluate a trained latent VAE Burgers model")
    parser.add_argument(
        "--dataset-path",
        type=str,
    ...
    parser.add_argument(
        "--checkpoint-path",
        type=str,
        default="grad_flow_l2/burgers/outputs_vae/run_latest/best_model.pt",
    ...
    parser.add_argument("--output-dir", type=str, default="grad_flow_l2/burgers/outputs_vae/eval")
...
def _display_fields(u: torch.Tensor, boundary_condition: str) -> tuple[torch.Tensor, torch.Tensor]:
    ...
    n_x = int(u.shape[-1])
    x = torch.arange(n_x, dtype=u.dtype) / float(n_x)
    return u, x
...
def _plot_sample_comparisons(
    ...
    for sample_id in sample_ids:
        ...
        u_ref_plot, x = _display_fields(u_ref, boundary_condition)
        u_pred_plot, _ = _display_fields(u_pred, boundary_condition)
        ...
        f_plot = f[sample_id].cpu()
        if boundary_condition == "dirichlet" and f_plot.shape[-1] == u_ref.shape[-1]:
            f_plot = pad_dirichlet_1d(f_plot)
        if f_plot.shape[-1] == x.numel():
            axes[0, 0].plot(x.numpy(), f_plot.numpy(), color="tab:green", linewidth=1.8)
        else:
            x_f = torch.arange(f_plot.shape[-1], dtype=f_plot.dtype) / float(f_plot.shape[-1])
            axes[0, 0].plot(x_f.numpy(), f_plot.numpy(), color="tab:green", linewidth=1.8)
        axes[0, 0].set_title("forcing")
        axes[0, 0].set_xlabel("x")
        axes[0, 0].set_ylabel("f")
        axes[0, 0].grid(alpha=0.25)

        axes[0, 1].plot(x.numpy(), u_ref_plot[0].numpy(), color="black", linewidth=1.8)
        ...
        for j, t_snap in enumerate(snapshot_times):
            ...
            axes[0, 2].plot(x.numpy(), u_ref_plot[k].numpy(), color=color, linewidth=1.7, label=f"true t={t_actual:g}")
            axes[0, 2].plot(
                x.numpy(),
                u_pred_plot[k].numpy(),
                color=color,
        ...
        fig.suptitle(
            f"Burgers VAE sample {sample_id}: mean rel L2={rel_mean:.3e}, final rel L2={rel_final:.3e}",
            fontsize=13,
        )
