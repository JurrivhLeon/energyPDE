"""
Evaluation script for unforced 1D Kuramoto-Sivashinsky latent VAE models.
...
try:
    ...
    from grad_flow_l2.ks1d.train_vae import rollout_vae_mean
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Evaluate a trained latent VAE KS model")
    parser.add_argument(
        "--dataset-path",
        type=str,
    ...
    parser.add_argument(
        "--checkpoint-path",
        type=str,
        default="grad_flow_l2/ks1d/outputs_vae/run_latest/best_model.pt",
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=True,
        help="Compatibility flag; KS VAE evaluation always disables the forcing channel.",
    )
    ...
    parser.add_argument("--output-dir", type=str, default="grad_flow_l2/ks1d/outputs_vae/eval")
...
def _plot_sample_comparisons(
    ...
    for sample_id in sample_ids:
        ...
        fig.suptitle(
            f"KS VAE sample {sample_id}: mean rel L2={rel_mean:.3e}, final rel L2={rel_final:.3e}",
            fontsize=13,
        )
...
def main(args: argparse.Namespace) -> None:
    ...
    print(
        ...
        f"h={h:.6f}, dt={dt:.6f}, boundary_condition={boundary_condition}, L={domain_length:.6f}"
    )
