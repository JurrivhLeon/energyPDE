"""Evaluation entrypoint for periodic 2D MHD latent VAE checkpoints."""
...
try:
    from ..mhd2d.mhd_data import build_mhd2d_step_dataset, build_mhd2d_trajectory_dataset_from_split
    from ..mhd2d.eval import (
    ...
    from .vae_trainer import PeriodicLatentVAETrainer2D
    from .train_vae import _build_model
except ImportError:
    from grad_flow_l2.mhd2d.mhd_data import build_mhd2d_step_dataset, build_mhd2d_trajectory_dataset_from_split
    from grad_flow_l2.mhd2d.eval import (
        _evaluate_rollout_curves,
        _parse_snapshot_times,
        _plot_curve,
        _plot_samples,
    ...
    from grad_flow_l2.mhd2d.vae_trainer import PeriodicLatentVAETrainer2D
    from grad_flow_l2.mhd2d.train_vae import _build_model


# Stored MHD state channels.
CHANNEL_NAMES = ["omega", "a"]

# Both channels are mean-free signed fields, so use symmetric diverging colormaps.
_CHANNEL_CMAP = {"omega": "RdBu_r", "a": "RdBu_r"}
_CHANNEL_SYMMETRIC = {"omega": True, "a": True}


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Evaluate periodic 2D MHD latent VAE checkpoint")
    parser.add_argument("--dataset-path",   type=str, required=True)
    parser.add_argument("--checkpoint",     type=str, required=True)
    parser.add_argument("--split",          type=str, default="val", choices=["train", "val", "test"])
    parser.add_argument("--output-dir",     type=str, default="grad_flow_l2/mhd2d/outputs_vae/eval")
...
def main(args: argparse.Namespace) -> None:
    ...
    step_loader = DataLoader(build_mhd2d_step_dataset(split),
                             batch_size=args.batch_size, shuffle=False, num_workers=args.num_workers)
    traj_loader = DataLoader(build_mhd2d_trajectory_dataset_from_split(split),
                             batch_size=args.batch_size, shuffle=False, num_workers=args.num_workers)
