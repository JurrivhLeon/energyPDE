"""Training entrypoint for periodic 2D MHD latent Markov models."""
...
try:
    from ..mhd2d.mhd_data import STATE_CHANNELS, build_mhd2d_step_dataset, build_mhd2d_trajectory_dataset_from_split
    ...
    from ..latent_markov_trainer_mc import LatentMarkovTrainer2D
except ImportError:
    from grad_flow_l2.mhd2d.mhd_data import STATE_CHANNELS, build_mhd2d_step_dataset, build_mhd2d_trajectory_dataset_from_split
    ...
    from grad_flow_l2.latent_markov_trainer_mc import LatentMarkovTrainer2D
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train latent Markov model on periodic 2D MHD data")
    ...
    parser.add_argument("--transition-type",   type=str,   default="fno", choices=["cnn", "fno"])
    ...
    parser.add_argument("--cnn-blocks",        type=int,   default=6,
                        help="Number of blocks for CNN transition (ignored for fno)")
    ...
    parser.add_argument("--channel-weights", type=float, nargs=2, default=None,
                        metavar=("W_OMEGA", "W_A"),
    ...
    parser.add_argument("--output-dir",     type=str,  default="grad_flow_l2/mhd2d/outputs")
    parser.add_argument("--dry-run",        action="store_true")
    return parser.parse_args()


def _build_model(n_x: int, n_y: int, dt: float,
                 args: argparse.Namespace) -> LatentMarkovModel2D:
    bc = "periodic"
...
def main(args: argparse.Namespace) -> None:
    ...
    dt = float(meta.get("record_dt", float(meta.get("t_final", float(n_steps))) / float(n_steps)))
    h_x, h_y = 1.0 / float(n_x), 1.0 / float(n_y)

    # Per-channel loss weights: 1/Var from training data (auto) or user-specified
    ...
    print(f"Channel weights (omega,a): {channel_weights.tolist()}")

    train_step_loader = DataLoader(build_mhd2d_step_dataset(train_split), batch_size=args.batch_size, shuffle=True,  num_workers=args.num_workers)
    val_step_loader   = DataLoader(build_mhd2d_step_dataset(val_split),   batch_size=args.batch_size, shuffle=False, num_workers=args.num_workers)
    test_step_loader  = DataLoader(build_mhd2d_step_dataset(test_split),  batch_size=args.batch_size, shuffle=False, num_workers=args.num_workers)
    val_traj_loader   = DataLoader(build_mhd2d_trajectory_dataset_from_split(val_split),   batch_size=args.batch_size, shuffle=False, num_workers=args.num_workers)
    test_traj_loader  = DataLoader(build_mhd2d_trajectory_dataset_from_split(test_split),  batch_size=args.batch_size, shuffle=False, num_workers=args.num_workers)
    ...
    trainer = LatentMarkovTrainer2D(
        model=model, dt=dt, h_x=h_x, h_y=h_y,
