"""Training entrypoint for periodic 2D MHD latent VAE models."""
...
try:
    ...
    from .vae_trainer import PeriodicLatentVAETrainer2D, _unpack_step_batch
except ImportError:
    ...
    from grad_flow_l2.mhd2d.vae_trainer import PeriodicLatentVAETrainer2D, _unpack_step_batch
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train latent VAE model on periodic 2D MHD data")
    ...
    parser.add_argument("--channel-weights", type=float, nargs=2, default=None,
                        metavar=("W_OMEGA", "W_A"),
                        help="Per-channel loss weights (omega a). "
                             "Default: inverse-std weights computed from training data.")
    parser.add_argument("--seed",           type=int,  default=42)
    parser.add_argument("--cpu",            action="store_true")
    parser.add_argument("--no-epoch-pbar",  action="store_true")
    parser.add_argument("--output-dir",     type=str,  default="grad_flow_l2/mhd2d/outputs_vae")
...
def _assert_dry_run_shapes(model: PeriodicLatentVAE2D, loader: DataLoader,
                            device: str, dt: float) -> None:
    model.eval()
    u_k, _, f = _unpack_step_batch(next(iter(loader)))
    u_k, f = u_k.to(device), f.to(device)
    with torch.no_grad():
...
def main(args: argparse.Namespace) -> None:
    ...
    trainer = PeriodicLatentVAETrainer2D(
        ...
        beta_kl=args.beta_kl, lambda_rec=args.lambda_rec,
